...
create table Общая 
( 
Номер_автозаправки int not null constraint номер_РК primary key, 
Адреса nchar(50), 
) 
...
create table Запись 
( 
Номер_записи int not null constraint номерp_РК primary key, 
Номер_автозаправки int not null constraint номерзз_РК foreign key references Общая(Номер_автозаправки), 
Тип_топлива int not null, 
) 
...
( 
Код int not null constraint PK_Код primary key, 
Номер_автозаправки int not null constraint PK_Автозаправка foreign key references Общая(Номер_автозаправки), 
...
values (1, 1,80298887766,N'Автозаправка в центре города.Гарантированная продажа
качественных нефтепродуктов и всевозможных товаров и услуг для вас и вашей машины',1,1,NULL,1, 5,1,1,NULL,NULL,1); 
insert into Автозаправка(Код, Номер_автозаправки, Номер_телефона,Описание,Номер_карточки_клиента,Акции_на_товар,Акции_на_услугу,код_топлива,количесвто_топлива,код_товара,количесвто_товаров,код_услуги,количесвто_услуг,код_чека) 
values (2,2,80298887765,N'Автозаправка на выезде из города.Гарантированная продажа качественных нефтепродуктов и всевозможных товаров и услуг для вас и вашей машины',2,NULL,2,2, 6,NULL,NULL,1,1,1); 
create table Сервер 
( 
